...
int main(int argc, char* argv[]){
    ...
    if(argc == 1){
        ...
        for(int i = 0; i < metadata.height; i++){ //so avança altura quando a linha toda ja foi preenchida
                ...
                metadata.matriz_de_pixels[i][j].r = g;
                metadata.matriz_de_pixels[i][j].g = g;
                metadata.matriz_de_pixels[i][j].b = g;
    ...
    if(argc == 2){
        ...
        for(int i = 0; i < metadata.height; i++){ //so avança altura quando a linha toda ja foi preenchida
                ...
                //scnf("%d %d %d", &metadata.matriz_de_pixels[i-1][j-1].r, &metadata.matriz_de_pixels[i-1][j-1].g, &metadata.matriz_de_pixels[i-1][j-1].b); // inverter a linha, mas mantem as colunas em sequencia de leitura
            }
            fclose(pf);
            imprimir_prompt(metadata);
